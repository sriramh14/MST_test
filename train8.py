"""Train the residual-diffusion refinement model on paired RGB/HSI data.

MST++ is loaded from a pretrained checkpoint and remains frozen. The trainable
network predicts the residual

    ground_truth_hsi - mst_prediction

and the final reconstruction is

    mst_prediction + predicted_residual.

The script intentionally keeps the structure of the supplied training script.
Only model-specific, loss-specific, checkpoint, sampling, and visualization
parts were changed.
"""
...
from model.RBBDM_rgb2hsi import MSTResidualDiffusion
...
OUTPUT_DIR = "./mst_residual_diffusion_checkpoints"
...
INFERENCE_CHECKPOINT = (
    "./mst_residual_diffusion_checkpoints/best_residual_diffusion.pth"
)
...
INFERENCE_OUTPUT_DIR = "./mst_residual_diffusion_inference"
...
# Set to None to leave sampled HSI values unchanged.
# Example for normalized data: (0.0, 1.0)
INFERENCE_CLAMP_RANGE: Optional[Tuple[float, float]] = None

# Residual-diffusion architecture.
HSI_CHANNELS = 31
RGB_CHANNELS = 3
...
# The residual U-Net has two stride-2 downsamples.
...
def default_model_config() -> dict:
    return {
        ...
        "base_channels": RESIDUAL_BASE_CHANNELS,
...
def build_residual_diffusion(
    ...
    model = MSTResidualDiffusion(
        ...
        kappa=config["kappa"],
        strict_checkpoint=STRICT_MST_CHECKPOINT,
    )
...
def save_checkpoint(
    ...
    torch.save(
        {
            "epoch": epoch,
            # This mirrors the supplied script and makes the saved checkpoint
            # self-contained. MST++ remains frozen during optimization.
    ...
    for batch_index, (hsi, rgb) in enumerate(
        ...
        if not torch.isfinite(gradient_norm):
            ...
            print(
                ...
                f"|noisy residual|max="
                f"{float(outputs['noisy_residual'].detach().abs().max()):.4g}, "
        ...
        update_timestep_tracker(
            ...
            timestep=outputs["timestep"].detach(),
...
def validate_reconstruction_metrics(
        ...
        for hsi, rgb in loader:
            ...
            with autocast_context(
                ...
                sample_outputs = model.sample(
                    rgb=rgb,
                    clamp_range=(
                        INFERENCE_CLAMP_RANGE
                    ),
                )

            refined_accumulator.update(
                prediction=sample_outputs[
                    "reconstruction"
                ],
                target=hsi,
            )
    ...
    result = {
        **refined_metrics,
        ...
        "sampling_steps": model.num_steps,
    }
    result.update(
        {
            f"mst_{key}": value
    ...
    metrics_path.write_text(
        "\n".join(
            [
                ...
                f"sampling_steps={model.num_steps}",
...
def parse_command_line_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train or evaluate the frozen-MST++ residual diffusion model."
        )
    )
    parser.add_argument(
        "--visualize",
