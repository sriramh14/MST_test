"""Train an Image-to-Image Schrodinger Bridge (I2SB) RGB -> HSI model from
paired RGB/HSI images.
...
6. Images are padded only immediately before the model forward pass so their
   height and width are compatible with the epsilon-network's downsampling
   factor. Every prediction is cropped back to the original size before
   losses and metrics are calculated.
7. This model is `I2SBModel` from `i2sb_hsi_model.py`: an Image-to-Image
   Schrodinger Bridge [Liu et al., 2023, ICML] between the clean HSI
   distribution (boundary X0, ground truth) and the degraded HSI
   distribution (boundary X1, the frozen MST++ prediction). Concretely,
   per training step (Algorithm 1 of the paper):
       t  ~ U([0, 1])
       X1 = MST++(rgb)                              (frozen backbone)
       Xt ~ q(Xt | X0, X1)                            (Proposition 3.3,
                                                        model.bridge_posterior_sample)
       loss = || eps(Xt, t, X1; theta) - (Xt - X0) / sigma_t ||^2   (Eq. 12)
   A spectral-angle regularisation term is additionally applied (with its
   own warm-up schedule) to the network's cheap single-step X0 estimate
   (`model.predict_x0_from_eps`), because it is a well known,
   differentiable, HSI-specific regulariser that discourages spectral
   distortion introduced by a pixel-wise objective alone:
       loss = loss + sam_weight(epoch) * spectral_angle_loss(hsi_gt, pred_x0)
8. Regardless of the training loss, mrae / rmse / sam / psnr / ssim are always
   computed every epoch for the coarse network's prediction (y0 = X1) and for
   the model's cheap single-step reconstruction estimate (pred_x0, obtained
   from a single randomly sampled bridge time t via
   `model.predict_x0_from_eps`). The fully bridge-refined output (the
   multi-step DDPM-style reverse process of Algorithm 2,
   `model.reverse_sample`) is only run ONCE, after the final training epoch,
   since it is far more expensive; its metrics are saved in the final
   checkpoint and printed at the end of the run.
9. After training, an inference routine randomly selects a handful of RGB
   images from a directory, runs the complete RGB -> MST++ -> I2SB reverse
   sampling pipeline on each, and saves the refined HSI reconstruction (and
   optionally the MST++ coarse prediction) to disk.

Compatibility note
-------------------
This script targets the attached `i2sb_hsi_model.py` implementation and
uses only its public API:
    model.backbone.mstpp_model      -- frozen MST++ instance (weights loaded
                                        externally, exactly as before)
    model.get_degraded_prediction   -- RGB -> X1 (frozen MST++ forward pass)
    model.bridge_posterior_sample   -- Xt ~ q(Xt|X0,X1), target eps (Prop 3.3)
    model.eps_net                   -- eps(Xt, t, X1; theta)
    model.predict_x0_from_eps       -- Xt, eps -> X0 estimate
    model.compute_loss              -- the same objective computed end to end
                                        (used implicitly; this script also
                                        exposes the intermediate pred_x0 for
                                        the "quick" metrics described above)
    model.reverse_sample / model.predict -- Algorithm 2 full reverse process
No internal I2SB logic (schedules, posteriors, network architecture) is
reimplemented here; this script only orchestrates data loading, padding,
metrics, checkpointing, and calls into the model's public methods.
"""
...
# TODO: adjust this import to wherever you saved the attached I2SB model
# file (the file that defines I2SBModel, I2SBScheduler, UNetEpsilonNet and
# MSTPPBackbone).
...
# Pretrained MST++ checkpoint. `I2SBModel` assumes MST++ weights are loaded
# externally (it only instantiates and freezes the network via
# `MSTPPBackbone`), so this training script loads the checkpoint into
# `model.backbone.mstpp_model` itself, right after the model is constructed.
# MST++ is frozen inside `MSTPPBackbone` itself (all parameters have
# requires_grad=False and it is always run under torch.no_grad() /
# forced into eval()) -- no additional freezing logic is required here.
MST_CKPT_PATH = None

# I2SB bridge scheduler (I2SBScheduler). These values must match
# model.i2sb_hsi_model.I2SBScheduler / I2SBModel's own defaults.
I2SB_NUM_TRAIN_TIMESTEPS = 1000
I2SB_BETA_MIN = 1.0e-6
I2SB_BETA_MAX = 1.2e-4

# Epsilon-prediction U-Net (UNetEpsilonNet) architecture. These values must
# match model.i2sb_hsi_model.UNetEpsilonNet.
UNET_BASE_CHANNELS = 64
UNET_CHANNEL_MULTIPLIERS = (1, 2, 4)
UNET_NUM_RES_BLOCKS = 2
UNET_TIME_DIM = 256
UNET_CONDITION_ON_X1 = True
UNET_ATTENTION_AT_BOTTLENECK = True

# Reverse (generation) sampling. I2SB's Algorithm 2 is a DDPM-style discrete
# reverse process over an analytic posterior; the paper does not use DDIM.
# The number of discretization steps used both for the (expensive,
# end-of-training-only) full validation reverse pass and for the inference
# routine below.
NUM_SAMPLING_STEPS = 50

# The epsilon U-Net downsamples (len(UNET_CHANNEL_MULTIPLIERS) - 1) times by
...
# Spectral-angle regularisation on the cheap single-step X0 estimate,
# ramped up over training (mirrors the old KL-beta warm-up, but there is no
# latent code here). I2SB's own objective (Eq. 12) is a fixed MSE on eps and
# is not separately configurable; this term is added on top of it.
SAM_LOSS_WEIGHT_START = 0.0
SAM_LOSS_WEIGHT_END = 0.00
SAM_WARMUP_EPOCHS = 30

# Full reverse-diffusion sampling (multi-step Algorithm 2 through the
# epsilon U-Net, per validation image) is expensive, so it is only run once,
# after the final training epoch, to report the model's true inference-time
...
# epoch.
VALIDATION_CACHE = Path(OUTPUT_DIR) / "i2sb_validation_cache.pth"
HSI_CHECKER_VERSION = "i2sb-rgb-hsi-pair-cache-v1"

# Post-training inference: randomly sample a handful of RGB images from this
# directory and run the complete RGB -> MST++ -> I2SB reverse-sampling
# pipeline on each, saving the results to INFERENCE_OUTPUT_DIR.
INFERENCE_RGB_DIR = RGB_DATA_DIR
INFERENCE_OUTPUT_DIR = Path(OUTPUT_DIR) / "inference_samples"
INFERENCE_NUM_IMAGES = 5
INFERENCE_NUM_SAMPLING_STEPS = NUM_SAMPLING_STEPS
SAVE_COARSE_PREDICTION = True
...
def i2sb_training_step(
    model: I2SBModel,
    rgb: torch.Tensor,
    hsi_gt: torch.Tensor,
    sam_weight: float,
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Run one I2SB training step and return (loss, pred_x0, y0), all
    cropped back to the input's original spatial size.

    This calls the model's public primitives in the same order as
    `I2SBModel.compute_loss` (get_degraded_prediction -> bridge_posterior_sample
    -> eps_net -> MSE on eps, Eq. 12), but additionally keeps the cheap
    single-step X0 estimate (`model.predict_x0_from_eps`) around so it can
    be used both for the optional spectral-angle regulariser and for the
    "quick" per-epoch metrics described at the top of this file.

    `y0` here is the frozen MST++ coarse prediction (the bridge's X1
    boundary). `pred_x0` here is the network's cheap single-timestep X0
    estimate at a randomly sampled bridge time t.
    """
    ...
    b = padded_hsi.shape[0]
    t = torch.rand(b, device=padded_hsi.device, dtype=padded_hsi.dtype)

    # Proposition 3.3: analytic bridge posterior sample Xt ~ q(Xt|X0,X1)
    # together with its regression target (Xt - X0) / sigma_t.
    xt_padded, target_eps_padded = model.bridge_posterior_sample(padded_hsi, x1_padded, t)
    cond = x1_padded if model.condition_on_x1 else None
    pred_eps_padded = model.eps_net(xt_padded, t, cond=cond)

    # footnote 1 of the paper: X0_pred = Xt - sigma_t * eps.
    pred_x0_padded = model.predict_x0_from_eps(xt_padded, t, pred_eps_padded)

    target_eps = target_eps_padded[..., :height, :width]
    pred_eps = pred_eps_padded[..., :height, :width]
    y0 = x1_padded[..., :height, :width]
    pred_x0 = pred_x0_padded[..., :height, :width]

    # Eq. (12): I2SB's own rescaled denoising objective.
    loss = F.mse_loss(pred_eps, target_eps)
    if sam_weight > 0:
        # The spectral-angle regulariser is computed on the cheap X0
        # estimate (absolute spectral vectors), not on eps, since spectral
        # angle is only meaningful on absolute spectral vectors.
...
@torch.no_grad()
def i2sb_full_sample(
    model: I2SBModel,
    rgb: torch.Tensor,
) -> Tuple[torch.Tensor, torch.Tensor]:
    """Run the full DDPM-style reverse process (Algorithm 2) and crop back
    to size."""
    padded_rgb, (height, width) = pad_to_multiple(rgb, MODEL_DOWNSAMPLE_FACTOR)

    x1_padded = model.get_degraded_prediction(padded_rgb)
    cond = x1_padded if model.condition_on_x1 else None
    fine_hsi_padded = model.reverse_sample(
        x1_padded, cond=cond, num_steps=NUM_SAMPLING_STEPS
    )
...
def train_one_epoch(
    ...
    for batch_index, (rgb, hsi) in enumerate(loader, start=1):
        ...
        with autocast(enabled=use_amp):
            loss, pred_x0, _y0 = i2sb_training_step(model, rgb, hsi, sam_weight)
        ...
        # pred_x0 is the I2SB model's single-step X0 estimate at a random
        # bridge timestep, not the fully sampled output. It is a fast, noisy
        # but useful proxy for tracking whether the epsilon U-Net is
        # learning.
...
def validate(
    ...
    -------
    quick_metrics:
        I2SB training loss and single-step X0 estimate metrics (cheap,
        always computed, comparable across every epoch).
    coarse_metrics:
        mrae/rmse/sam/psnr/ssim of the frozen MST++ network's output y0
        (the bridge's X1 boundary) alone, for reference.
    full_sample_metrics:
        mrae/rmse/sam/psnr/ssim of the fully bridge-refined output,
        obtained by running the complete multi-step reverse sampling loop
        (Algorithm 2). Only populated when run_full_sampling is True;
        otherwise returned empty.
    """
    model.eval()
    ...
    for rgb, hsi in loader:
        ...
        with autocast(enabled=use_amp):
            loss, pred_x0, y0 = i2sb_training_step(model, rgb, hsi, sam_weight)
        ...
        if run_full_sampling:
            fine_hsi, _y0_full = i2sb_full_sample(model, rgb)
    ...
    torch.save(
        {
            ...
            "model_config": {
                ...
                "condition_on_x1": UNET_CONDITION_ON_X1,
                "attn_at_bottleneck": UNET_ATTENTION_AT_BOTTLENECK,
                "num_train_timesteps": I2SB_NUM_TRAIN_TIMESTEPS,
                "beta_min": I2SB_BETA_MIN,
                "beta_max": I2SB_BETA_MAX,
            },
            "training_config": {
...
def run_inference_on_random_images(
    ...
    """Randomly select `num_images` RGB images from `rgb_dir`, run the full
    RGB -> MST++ (X1) -> I2SB reverse-sampling pipeline (Algorithm 2) on
    each, and save the refined HSI reconstruction (and optionally the
    coarse MST++ prediction) as `.npy` cubes of shape [C,H,W].
    """
    model.eval()
    ...
    chosen_files = rng.sample(rgb_files, k=min(num_images, len(rgb_files)))

    print(
        f"\nRunning I2SB inference on {len(chosen_files)} randomly selected "
        f"RGB image(s) from {rgb_dir} ..."
    )

    for rgb_path in chosen_files:
        rgb_array = load_rgb_file(rgb_path)
        rgb_tensor = torch.from_numpy(np.ascontiguousarray(rgb_array)).float()
        ...
        # RGB -> frozen MST++ -> X1 (the bridge's degraded boundary).
        x1_padded = model.get_degraded_prediction(padded_rgb)
        cond = x1_padded if model.condition_on_x1 else None

        # Algorithm 2: full DDPM-style reverse process, X1 -> X0.
        fine_hsi_padded = model.reverse_sample(
            x1_padded, cond=cond, num_steps=num_sampling_steps
        )

        fine_hsi = fine_hsi_padded[..., :height, :width].squeeze(0).cpu().numpy()
        y0 = x1_padded[..., :height, :width].squeeze(0).cpu().numpy()

        stem = rgb_path.stem
        reconstruction_path = output_dir / f"{stem}_i2sb_reconstruction.npy"
        np.save(reconstruction_path, fine_hsi)

        if save_coarse:
            coarse_path = output_dir / f"{stem}_mst_coarse_prediction.npy"
            np.save(coarse_path, y0)

        print(f"  {rgb_path.name} -> {reconstruction_path}")
...
def main() -> None:
    ...
    if MST_CKPT_PATH is not None:
        # I2SBModel (via MSTPPBackbone) does not load MST++ weights itself
        # (by design, per its own spec) -- it only instantiates and freezes
        # the network. The training script is therefore responsible for
        # loading the pretrained MST++ checkpoint into
        # model.backbone.mstpp_model.
        ...
        model.backbone.mstpp_model.load_state_dict(mst_state_dict, strict=True)
    ...
    # mode) as soon as MSTPPBackbone is constructed with freeze=True. This
    # re-assertion after loading external weights adds no new freezing
    # behaviour; it just guards against the checkpoint having stored
    # requires_grad state.
    ...
    print(
        f"Trainable parameters (I2SB epsilon U-Net): "
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        # Full multi-step reverse-diffusion sampling (Algorithm 2) only runs
        # after the final epoch, since it is far more expensive than the
        # single-step quick_metrics computed every epoch below.
        ...
        if is_last_epoch:
            print(
                f"  Full I2SB reverse sample (final epoch) | "
        ...
        save_checkpoint(
            output_path=output_dir / "last_i2sb.pth",
            model=model,
            optimizer=optimizer,
        ...
        if quick_metrics["mrae"] < best_validation_mrae:
            ...
            save_checkpoint(
                output_path=output_dir / "best_i2sb.pth",
                model=model,
                optimizer=optimizer,
                lr_scheduler=lr_scheduler,
                scaler=scaler,
        ...
        if is_last_epoch:
            save_checkpoint(
                output_path=output_dir / "final_i2sb_with_full_sampling.pth",
                model=model,
                optimizer=optimizer,
                lr_scheduler=lr_scheduler,
                scaler=scaler,
            ...
            print(
                ...
                f"PSNR: {full_sample_metrics['psnr']:.4f} | "
                f"SSIM: {full_sample_metrics['ssim']:.4f}\n"
                f"Saved to: {output_dir / 'final_i2sb_with_full_sampling.pth'}"
            )

    # Post-training inference on a handful of randomly selected RGB images.
    run_inference_on_random_images(
        model=model,
        rgb_dir=INFERENCE_RGB_DIR,
        output_dir=INFERENCE_OUTPUT_DIR,
        num_images=INFERENCE_NUM_IMAGES,
        device=device,
        num_sampling_steps=INFERENCE_NUM_SAMPLING_STEPS,
        save_coarse=SAVE_COARSE_PREDICTION,
        seed=SEED,
    )
